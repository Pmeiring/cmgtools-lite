...
def addMakerOptions(parser):
	...
	parser.add_option("--smcfriends" , dest="smcfriends" , action="append", default=[], help="Overwriting (!) the smcfriends variable from the config");
	...
	parser.add_option("--replInMccs", dest="replInMccs", type="string" , action="append", default=[], help="Replace string in MCCs");
	return parser
...
class Maker():
	def __init__(self, module, base, args, options, defaults):
		...
		print "Using instance",self.instance
	...
	def collectMCCs(self):
		...
		if len(self.options.replInMccs)>0: 
			for repl in self.options.replInMccs:
				sr = repl.split(":")
				use = [m.replace(sr[0],sr[1]) for m in use]
	...
	def getFriends(self,isFastSim=False):
		...
		friends += ["--FMC sf/t {P}/"+f+"/evVarFriend_{cname}.root"  for f in getattr(self.options, "smcfriends", [])] if len(self.options.smcfriends)>0 else ["--FMC sf/t {P}/"+f+"/evVarFriend_{cname}.root"     for f in getattr(self.config,"smcfriends"   ,[])]
